package solutions.question888a;
...
public class Example {
  ...
  static <S, T> List<ImmutablePair<S, T>> flatten(
    ...
    return maybePairs.stream()
        .map(
            p ->
                new ImmutablePair<>(
                    p.getFirst().orElse(defaultS), p.getSecond().orElse(defaultT)))
        .collect(Collectors.toList());
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Zipped string and float lists after flattening: " + flattenedZippedStringAndFloatLists);
  }
}
